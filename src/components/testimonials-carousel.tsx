...
export function TestimonialsCarousel({
    ...
    const fetchTestimonials = async () => {
      try {
        ...
        const testimonialsFormatted: Testimonial[] = rawTestimonialsArray
          .filter(
            (item: unknown): item is ApiResponse =>
              item !== null &&
              typeof item === "object" &&
              "id" in item &&
              "name" in item &&
        ...
        // 👉 Seleccionar 12 aleatorios
        const limitedRandom = shuffleArray(testimonialsFormatted).slice(0, 12);

        setTestimonials(limitedRandom);
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : "Error al cargar testimonios";
  ...
  return (
    ...
    >
      <h2 className="text-2xl lg:text-3xl font-bold mb-8 text-white text-center">
        {title}
      </h2>
      <div
        className="relative"
      ...
      >
        ...
        <button
          onClick={handlePrev}
          className="hidden md:absolute md:-left-16 md:top-1/2 md:-translate-y-1/2 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full shadow-md transition-colors md:flex md:items-center md:justify-center"
        ...
        <button
          onClick={handleNext}
          className="hidden md:absolute md:-right-16 md:top-1/2 md:-translate-y-1/2 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full shadow-md transition-colors md:flex md:items-center md:justify-center"
